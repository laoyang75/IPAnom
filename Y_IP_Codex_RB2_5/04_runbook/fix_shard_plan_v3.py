...
-- 断言
SELECT CASE WHEN (SELECT total_rows FROM sp_params) >= 64 THEN 1 ELSE 1/0 END AS assert_rows_ge_64;
...
-- 断言
SELECT CASE WHEN (SELECT COUNT(*) FROM sp_agg) = 64 THEN 1 ELSE 1/0 END AS assert_shard_cnt_64;
SELECT CASE WHEN EXISTS (SELECT 1 FROM sp_agg WHERE cnt = 0) THEN 1/0 ELSE 1 END AS assert_no_empty_shard;
...
-- 断言
SELECT CASE WHEN EXISTS (SELECT 1 FROM sp_bounds WHERE ip_long_start >= ip_long_end) THEN 1/0 ELSE 1 END AS assert_nonempty_range;
...
def exec_sql_embedded():
    ...
    temp_filename = f"/tmp/rb20_fix_v3_{int(time.time()*1000)}.sql"
    with open(temp_filename, 'w', encoding='utf-8') as f:
        f.write(formatted_sql)
        
    try:
        log(f"Executing RB20_00D fix v3 (No Transaction) with RUN_ID={RUN_ID}...")
        # Add -a to echo query
